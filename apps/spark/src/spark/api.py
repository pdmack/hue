...
class OozieApi:
  ...
  RE_LOG_START_RUNNING = re.compile('>>> Invoking Shell command line now >>(.+?)(<<< Invocation of Shell command completed <<<|<<< Invocation of Main class completed)', re.M | re.DOTALL)
  ...
  def _create_workflow(self, pig_script, params):
    ...
    script_path = workflow.deployment_dir + '/spark.py'
    script_path2 = workflow.deployment_dir + '/spark.sh'
    self.fs.do_as_user(self.user.username, self.fs.create, script_path, data=pig_script.dict['script'])
    self.fs.do_as_user(self.user.username, self.fs.create, script_path2, data="""
#!/usr/bin/env bash

WORKSPACE=`pwd`
cd /home/romain/projects/spark-0.8.0-incubating-bin-cdh4
pyspark $WORKSPACE/spark.py
    """)
  ...
  def _build_parameters(self, params):
    pig_params = [] #{"type": "argument", "value": "$PWD"}]
