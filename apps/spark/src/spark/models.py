...
from pig.models import Document



# TODO use Doc Model

class SparkScript(Document):
...
def create_or_update_script(id, name, script, user, parameters, resources, hadoopProperties, is_design=True):
  try:
    pig_script = SparkScript.objects.get(id=id)
    pig_script.doc.get().can_read_or_exception(user)
  except SparkScript.DoesNotExist:
    pig_script = SparkScript.objects.create(owner=user, is_design=is_design)
